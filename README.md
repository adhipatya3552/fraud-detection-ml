...
**An end-to-end Machine Learning system that detects fraudulent credit card transactions using real-world anonymized data — featuring a full modular ML pipeline, a FastAPI backend for real-time predictions, an interactive Streamlit web UI, and Docker support for deployment.**
...
> ⚠️ **The project will not run without this file.** `main.py` and `streamlit_app.py` both expect it at exactly this path.
...
Start the fraud detection REST API:
...
### Option 3 — Run the Streamlit Web App
...
The Streamlit app (`app/streamlit_app.py`) provides a visual interface for testing the fraud detection model without touching the API directly.

### What It Does

- Displays **29 number input fields** — one for each feature (V1–V28 + Amount)
- Offers **4 quick-action buttons** at the top:
  - 📥 **Normal Sample** — loads a real non-fraud transaction from the dataset into the fields
  - ⚠️ **Fraud Sample** — loads a real fraud transaction from the dataset into the fields
  - 🔄 **Reset** — resets all fields to 0.0
  - 🎲 **Random Values** — fills all fields with random values between -5 and 5
- When you click **🚀 Predict Fraud**, it sends the 29 values to the FastAPI `/predict` endpoint
...
The app reads directly from `data/creditcard_2023.csv` using `@st.cache_data` so the CSV is only loaded once. When you click "Normal Sample" or "Fraud Sample", it filters the dataset by `Class == 0` or `Class == 1`, picks a random row, strips the `id` and `Class` columns, and populates all 29 input fields with those real values.

This lets you instantly test what a real fraud or non-fraud transaction looks like to the model.

---

## 🐳 Docker Deployment

The project includes a `Dockerfile` that packages the FastAPI API into a container.
...
# Starts the FastAPI server on port 8000, accessible from outside the container
```
...
| **Streamlit and API must run separately** | Both processes need to be running simultaneously in separate terminals |
...
- [ ] Deploy the FastAPI backend to a cloud service (e.g., Railway, Render, or AWS EC2)
